...
- Tax answers are ledger-resolved only (ingest-time normalized rows with source+page provenance).
- Natural language parsing may map your phrasing to a metric, but values are never generated by the LLM.
- Default interpretation: “taxes paid” means federal income tax withheld (W-2 Box 2).
- If scope is missing (for example, no year) or values conflict, llmLibrarian abstains with a reason category (`ambiguous_scope`, `conflict`, `no_match`, `low_confidence_extraction`).
