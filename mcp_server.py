...
_chroma_lock = jobs._chroma_lock
_release_chroma = jobs.release_chroma
_mcp_chroma_lock = jobs.mcp_chroma_lock
_mcp_lock_timeout_seconds = jobs.mcp_lock_timeout_seconds
_acquire_chroma_lock = jobs.acquire_chroma_lock
_retry_after_seconds = jobs.retry_after_seconds
_mcp_read_lock_disabled = jobs.read_lock_disabled
_is_lock_timeout = jobs.is_lock_timeout
_compute_answer_confidence = diagnostics.compute_answer_confidence


def _busy_error(exc: BaseException, operation: str) -> dict:
    return jobs.busy_error(exc, operation, _DB_PATH)
...
def _healthz_payload() -> dict:
    ...
    if runtime.is_loopback_bind() or env_flag("LLMLIBRARIAN_MCP_REQUIRE_AUTH", False):
        payload["db_path"] = _DB_PATH
    else:
        payload["db_path_withheld"] = "bound off-loopback without LLMLIBRARIAN_MCP_REQUIRE_AUTH"
    return payload
