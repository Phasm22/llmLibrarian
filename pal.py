...
def _iter_editable_roots_early(site_root: Path) -> list[Path]:
    roots: list[Path] = []
...
def _default_project_root() -> Path:
    ...
    if _looks_like_checkout(cwd):
        return cwd
    if _looks_like_checkout(_PAL_ROOT):
        return _PAL_ROOT
    for editable_root in _iter_editable_roots_early(_PAL_ROOT):
        if _looks_like_checkout(editable_root):
            return editable_root
    return _PAL_ROOT


_DEFAULT_DB = str(_default_project_root() / "my_brain_db")

_ENV_TEMPLATE = """\
# llmLibrarian environment — generated by pal install
LLMLIBRARIAN_DB={db_path}
LLMLIBRARIAN_MODEL={model}

# Use ONNX runtime for embeddings (lighter idle, ~50MB vs ~800MB with torch).
# If you switch this on an existing index you must reindex: pal pull --full <folder>
LLMLIBRARIAN_EMBEDDING=default

# Keep torch thread pools small so idle CPU and memory stay low.
...
_IDLE_HINT = """\
           To reduce MCP server idle memory (~800MB → ~50MB), add to your env file:
             LLMLIBRARIAN_EMBEDDING=default  # requires reindex: pal pull --full <folder>
             CUDA_VISIBLE_DEVICES=
             OMP_NUM_THREADS=2
             TOKENIZERS_PARALLELISM=false"""
...
def _resolved_db_path() -> str:
    return str(Path(os.environ.get("LLMLIBRARIAN_DB", _DEFAULT_DB)).resolve())
...
def _run_llmli(args: list[str], extra_env: dict[str, str] | None = None) -> int:
    ...
    env = os.environ.copy()
    env.setdefault("LLMLIBRARIAN_DB", _resolved_db_path())
    if extra_env:
...
def _build_pull_env(status_file_path: str, *, quiet_subprocess: bool = True) -> dict[str, str]:
    ...
    for k, v in os.environ.items():
        if k in allow_exact or k.startswith("LLMLIBRARIAN_"):
            out[k] = v
    out.setdefault("LLMLIBRARIAN_DB", _resolved_db_path())
    _cli, src = _resolve_llmli_paths()
...
def install_command(
    ...
    # Resolve DB path
    xdg_data_raw = os.environ.get("XDG_DATA_HOME", "")
    xdg_data = Path(xdg_data_raw) if xdg_data_raw else home / ".local" / "share"
    resolved_db = Path(db_path).expanduser() if db_path else xdg_data / "llmLibrarian" / "db"

    # Create ~/.pal/ and logs/
    PAL_HOME.mkdir(parents=True, exist_ok=True)
    jobsrt.watch_log_dir(PAL_HOME).mkdir(parents=True, exist_ok=True)
    print(f"pal home:  {PAL_HOME}")
    ...
    # Write env file
    xdg_cfg_raw = os.environ.get("XDG_CONFIG_HOME", "")
    xdg_cfg = Path(xdg_cfg_raw) if xdg_cfg_raw else home / ".config"
    env_dir = xdg_cfg / "llmLibrarian"
    env_file = env_dir / ".llmlibrarian.env"
    legacy_env_file = env_dir / "llmlibrarian.env"

    existing_env_file = env_file if env_file.exists() else legacy_env_file if legacy_env_file.exists() else env_file
    if existing_env_file.exists() and not force:
        print(f"env file:  {existing_env_file} (already exists — use --force to overwrite)")
        if existing_env_file == legacy_env_file:
            print(f"           legacy path; prefer {env_file}")
        if not any(ln.startswith("LLMLIBRARIAN_EMBEDDING") for ln in existing_env_file.read_text().splitlines()):
            print(_IDLE_HINT)
    else:
        was_existing = env_file.exists()
        env_dir.mkdir(parents=True, exist_ok=True)
        env_file.write_text(_ENV_TEMPLATE.format(db_path=str(resolved_db), model=model), encoding="utf-8")
        env_file.chmod(0o600)
        print(f"env file:  {env_file} ({'overwrote' if was_existing else 'created'})")
    ...
    if _check_ollama_reachable():
        ...
        print(f"ollama:    not reachable at {host}", file=sys.stderr)
        print(f"           Install Ollama, then run:  ollama pull {model}", file=sys.stderr)

    # Install watch daemon services
    if not skip_daemon:
        manager_name = jobsrt.supported_service_manager()
        if not manager_name:
            print(f"daemon:    unsupported platform ({sys.platform}), skipping", file=sys.stderr)
        else:
            os.environ.setdefault("LLMLIBRARIAN_DB", str(resolved_db))
            try:
                metadata = _daemon_runtime_metadata()
                jobsrt.write_daemon_metadata(PAL_HOME, metadata)
                jobs, _ = _derive_watch_jobs_for_daemon(manager_name, db_path=str(resolved_db))
                if not jobs:
                    print(f"daemon:    ready ({manager_name}) — no silos yet, run `pal pull <folder>` to start watching")
                else:
                    rc = _sync_daemon_services(emit_output=False)
                    status = f"watching {len(jobs)} silo(s)" if rc == 0 else "installed with warnings (run `pal daemon sync`)"
                    print(f"daemon:    {status} ({manager_name})")
            except Exception as exc:
                print(f"daemon:    {exc}", file=sys.stderr)
    else:
        print("daemon:    skipped (--skip-daemon)")

    # Optionally install MCP service
    if mcp:
        manager_name = jobsrt.supported_service_manager()
        if not manager_name:
    ...
    print("")
    print("Next steps:")
    print("  1. Index a folder:  pal pull ~/Documents")
    print("  2. Ask a question:  pal ask 'what is X?'")
    if not skip_daemon:
        print("  3. Check services:  pal ls --status")
    if not mcp:
        print("  Tip: expose an MCP endpoint with:  pal install --mcp")
