...
# Helper to fetch silo root path from registry (if available)
...
def _run_income_year_total_guardrail(
    *,
    collection: Any,
    use_unified: bool,
    silo: str | None,
...
def _compute_project_count(
    *,
    ...
    # Gather paths either from registry or collection metas.
...
def _format_project_count(
    *,
    ...
    if count > 0:
        ...
        return "\n".join(lines)
    if samples:
        # code files at root but no distinct buckets
        lines = [
...
def run_ask(
    ...
    if intent == INTENT_MONEY_YEAR_TOTAL:
        guardrail = _run_income_year_total_guardrail(
            collection=collection,
            use_unified=use_unified,
            silo=silo,
            subscope_where=subscope_where,
    ...
    if intent == INTENT_PROJECT_COUNT:
        project_count, samples = _compute_project_count(db_path=db, silo=silo, collection=collection)
        response = _format_project_count(count=project_count, samples=samples, source_label=source_label, no_color=no_color)
        time_ms = (time.perf_counter() - t0) * 1000
        write_trace(
            intent=intent,
            n_stage1=0,
